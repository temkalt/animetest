...
export const VideoPlayerView: React.FC<VideoPlayerProps> = ({
  ...
  // Filtered translations by type
  const filteredTranslations = useMemo(() => {
    if (filterType === 'all') return currentTranslations;
    return currentTranslations.filter((t) => (filterType === 'sub' ? t.type === 'sub' : t.type !== 'sub'));
  }, [currentTranslations, filterType]);
  ...
  return (
    <div className={`space-y-4 ${isTheaterMode ? 'fixed inset-0 z-50 bg-[#07080B] p-4 sm:p-8 flex flex-col justify-between overflow-y-auto' : ''}`}>
      ...
      <div className="p-3.5 sm:p-4 rounded-2xl bg-[#0F1117] border border-white/[0.07] shadow-xl backdrop-blur-md space-y-3.5">
          ...
          <div className="flex items-center gap-1.5 p-1 rounded-xl bg-[#090A0E] border border-white/[0.05] overflow-x-auto scrollbar-none max-w-full">
            ...
            {probeData?.results?.anilibria?.available && (
              ...
              >
                ...
                <span>AniLibria</span>
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-white/10 text-cyan-200 font-mono">
                  1080p HLS
                </span>
              </button>
            )}
            ...
            >
              ...
              <span className="text-[10px] px-1.5 py-0.5 rounded bg-white/10 text-zinc-300 font-mono">
                {probeData?.results?.kodik?.translations?.length
                  ? `${probeData.results.kodik.translations.length} озвучек`
                  : 'Все студии'}
              </span>
            </button>
            ...
            >
              ...
              <span>Alloha</span>
              <span className="text-[10px] px-1.5 py-0.5 rounded bg-white/10 text-amber-200 font-mono">
                {probeData?.results?.alloha?.translations?.length
                  ? `${probeData.results.alloha.translations.length} студий`
                  : 'HD'}
              </span>
            </button>
            ...
            {probeData?.results?.collaps?.available && (
              ...
              >
                ...
                <span>Collaps</span>
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-white/10 text-emerald-200 font-mono">
                  Full HD
                </span>
              </button>
            )}
        ...
        {/* 2. Studio / Voiceover Selector (Clean Chip Grid) */}
        {currentTranslations.length > 1 && effectiveEngine !== 'multidub' && (
          <div className="pt-2 border-t border-white/[0.06] space-y-2">
            <div className="flex items-center justify-between text-xs text-zinc-400">
              <div className="flex items-center gap-2">
                <Volume2 className="w-3.5 h-3.5 text-indigo-400" />
                <span className="font-medium text-zinc-300">Доступные озвучки:</span>
              </div>

              {/* Dub / Sub Filter Chips */}
              <div className="flex items-center gap-1 bg-[#090A0E] p-0.5 rounded-lg border border-white/[0.05] text-[10px]">
                ...
                <button
                  ...
                  className={`px-2 py-0.5 rounded-md transition-all cursor-pointer ${
                    filterType === 'dub' ? 'bg-white/15 text-white font-medium' : 'text-zinc-400 hover:text-zinc-200'
                  }`}
                >
                  Дубляж
                </button>
                <button
                  ...
                  className={`px-2 py-0.5 rounded-md transition-all cursor-pointer ${
                    filterType === 'sub' ? 'bg-white/15 text-white font-medium' : 'text-zinc-400 hover:text-zinc-200'
                  }`}
                >
                  Субтитры
                </button>
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all cursor-pointer flex items-center gap-1.5 ${
                      isSelected
                        ? 'bg-indigo-600/20 text-indigo-200 border border-indigo-500/50 shadow-sm font-semibold'
                        : 'bg-[#090A0E] hover:bg-white/[0.06] text-zinc-300 border border-white/[0.05]'
                    }`}
                  >
      ...
      {/* 4. Bottom Info & Shortcuts Helper */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 px-4 py-3 rounded-xl bg-[#0F1117] border border-white/[0.05] text-xs">
        <div className="flex items-center gap-2.5">
          ...
          {activeTranslation && (
            <span className="hidden md:inline-block text-zinc-500">• {activeTranslation.teamName}</span>
          )}
        </div>
        ...
        <div className="flex items-center gap-4 text-zinc-400 text-[11px]">
          <div className="flex items-center gap-1.5 text-zinc-400">
            ...
            <span>При сбое переключите плеер на Kodik или Alloha выше</span>
          </div>
      ...
      {/* 5. Hotkeys Modal / Drawer */}
      {showHotkeys && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#12141C] border border-white/10 rounded-2xl p-6 max-w-sm w-full space-y-4 shadow-2xl">
