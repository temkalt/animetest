          ...
          {comments.map((comment, index) => {
            ...
            const safeUsername =
              comment.username && comment.username !== 'undefined'
                ? comment.username
                : 'kuronami';
            const safeAvatar = comment.userAvatar || DEFAULT_AVATARS[0];
            ...
            return (
              <motion.div
                ...
                className="flex gap-3 p-3.5 rounded-xl bg-zinc-900 border border-zinc-800 hover:border-zinc-700 hover:bg-zinc-800/50 transition-colors shadow-sm"
              >
                {/* User avatar */}
                <Link href={`/user/${safeUsername}`} className="shrink-0 group">
                  <div className="relative w-8 h-8 rounded-lg overflow-hidden bg-zinc-800 border border-zinc-700 group-hover:border-zinc-500 transition-colors">
                    <Image
                      src={safeAvatar}
                      alt={safeUsername}
                      fill
                ...
                <div className="flex-1 min-w-0 space-y-1">
                  ...
                  <p
                    ...
                    className={`text-sm text-zinc-300 line-clamp-2 ${
                      comment.isSpoiler
                        ? (isRevealed ? '' : 'blur-sm hover:blur-[2px]') +
                  ...
                  <Link
                    ...
                    className="inline-flex items-center gap-2 text-xs text-zinc-500 hover:text-zinc-300 transition-colors pt-0.5"
                  >
                    <div className="relative w-4 h-5 rounded-sm overflow-hidden bg-zinc-800 shrink-0 border border-zinc-750">
                      <Image
                    ...
                    {comment.episodeNumber && <span>· Серия {comment.episodeNumber}</span>}
                  </Link>
                </div>
              </motion.div>
            );
