...
export default function UserProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  useEffect(() => {
    const pub = authStore.getPublicProfile(username);
    setProfile(pub);
    const cols = authStore.getUserCollections(username);
  ...
  if (!profile) {
    return (
      <div className='min-h-[60vh] flex flex-col items-center justify-center gap-4'>
        ...
        <Link href='/' className='text-sm text-zinc-400 hover:text-zinc-100 transition-colors'>На главную</Link>
      </div>
    );
  }
  ...
  return (
    <div className='max-w-2xl mx-auto space-y-8 pb-16'>
      ...
      {/* Profile header */}
      <div className='flex items-start gap-5 p-6 rounded-lg bg-zinc-900 border border-zinc-800'>
        ...
        <div className='space-y-1.5'>
          ...
          <div className='flex items-center gap-3 pt-2 text-xs text-zinc-500'>
            <span className='px-2 py-0.5 rounded bg-zinc-800 border border-zinc-700 text-zinc-300'>{profile.role}</span>
      ...
      {collections.length > 0 && (
        <div className='space-y-4'>
          <div className='flex items-center gap-2'>
            ...
            <h2 className='text-lg font-semibold text-zinc-100'>Коллекции</h2>
            <span className='text-sm text-zinc-500'>{collections.length}</span>
          </div>
          <div className='space-y-2'>
            {collections.map((col) => (
              <div key={col.id} className='p-4 rounded-lg bg-zinc-900 border border-zinc-800 space-y-1'>
