...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const unsubscribeCols = authStore.subscribeCollections((cols) => {
      ...
      // Update active collection if open
        ...
        {isEditing && (
          <form onSubmit={handleSaveProfile} className="mt-6 pt-6 border-t border-zinc-800 space-y-4 animate-in fade-in">
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-mono text-zinc-400 uppercase">Аватар</label>
                ...
                {DEFAULT_AVATARS.map((av, idx) => (
                  <button
                    ...
                    className={`relative w-12 h-12 rounded-lg overflow-hidden border-2 shrink-0 transition-all cursor-pointer ${
                      editAvatar === av ? 'border-zinc-300 scale-105' : 'border-zinc-700 opacity-60 hover:opacity-100'
                    }`}
                  >
